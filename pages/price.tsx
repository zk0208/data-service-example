...
function Home1() {
  ...
  const LabelB = candleSticksByTime?.data.rows.slice(0,500).map((i) => i.time);
  ...
  const dataB = [
    ...
    const dataIndex = candleSticksByTime?.data.rows.slice(0,500).map((i) => [i.time, Number(i.open_price), Number(i.high_price), Number(i.low_price), Number(i.close_price)]);
    ...
    xAxis:{
      categories : candleSticksByTime?.data.rows.slice(0,500).map((i) => i.time),
    },
    rangeSelector : {
      buttons :[{
        type :'hour',
  ...
  const optionPrice:Highchartsl.Options ={
    ...
    series: [{
        type: 'area',
        name: 'price',
        data: candleSticksByTime?.data.rows.slice(0,500).map((i) => Number(i.close_price)),
    }]
  }
